...
class HomeScreen : Fragment() {
	...
	override fun onCreateView(
		...
		// Set appropriate overclock UI state
		when (viewModel.overclockState) {
			PlayerViewModel.OverclockState.ACTIVE ->
				binding.overclockText.text = resources.getString(R.string.Overclock_Active)
			PlayerViewModel.OverclockState.CHARGING ->
				binding.overclockText.text = resources.getString(R.string.Overclock_Charging)
			PlayerViewModel.OverclockState.READY -> {
				binding.overclockText.text = resources.getString(R.string.Overclock_Charged)
				binding.overclockBar.progress = 100
			}
		}
